...
const LineChart = props => {
	...
	useEffect(() => {
		if (props.data) {
			...
			const labelsMax = Math.min(totalPoints, 7);
...
chartSettings ?
	...
	<svg viewBox={`0 0 ${ chartSettings.width } ${ chartSettings.height }`}>
		<g transform={ `translate(0, ${ chartSettings.chartY })` }>
			...
			<path key={seriesIndex} d={ `M${ series.points.map(point => point.x + " " + point.y).join(" L") }` } stroke={ series.color } strokeDasharray={ series.isDashed ? 4 : 0 } className="linepath" />
			)
			}
