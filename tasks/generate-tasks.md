...
- **Format:** Markdown (`.md`)
- **Location:** Ask for the task directory
...
10.  **Save Task List:** Save the generated document in the same directory as the original PRD with the filename `[prd-file-name]-tasks.md`, where `[file-name-prd]` matches the base name of the input PRD file (e.g., if the input was `user-profile-editing-prd.md`, the output is `prd-user-profile-editing-tasks.md`).
...
- `path/to/another/file.js` - Brief description (e.g., API route handler for data submission).
