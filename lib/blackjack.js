...
export default class {
	...
	constructor(existingTransactions = [], startingBank = 200, defaultBet = 10) {
		strategy.display = "";
	...
	Deal = () => {
		...
		if (hands.player.value !== 21 && hands.dealer.value !== 21) {
			calculateStrategy(hands.player.cards, hands.dealer.cards);
		}
		else {
			let transaction = 0;
	...
	Play = action => {
		...
		if (hands.player.isComplete && (!hands.split || hands.split.isComplete)) {
			...
			if (hands.player.isBlackJack && !hands.dealer.value !== 21) {
				transaction += hands.player.bet * 2.5;
				hands.player.result = 1;
			}
			...
			if (hands.split) {
				if (hands.split.isBlackJack && !hands.dealer.value !== 21) {
			...
			calculateStrategy(hands.player.isComplete ? hands.split.cards : hands.player.cards, hands.dealer.cards);
		}
	};
